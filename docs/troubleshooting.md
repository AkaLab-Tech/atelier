...
**Fix:** This is almost always working as intended — finish or hand off the worktree you already own before touching the other one. If you're confident the other worktree's owning session is genuinely gone (crashed, killed, or its task already merged/abandoned) and you don't want to wait, the claim ages out on its own after `ownerTtlSeconds` in that worktree's `.atelier.json` (default 43200s / 12h) — a claim that old is reclaimed automatically by the next session that touches it. There's no in-session override; a deliberate cross-worktree edit is the operator's own call, made manually outside Claude Code. Inspect `<worktree>/.task-log/hook-decisions.jsonl` for `"hook":"block-cross-worktree-edit"` lines to see exactly which worktree collided and when.
