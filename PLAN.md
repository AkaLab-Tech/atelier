...
- `Bash(git push * main)`, `* master`, `* develop`, `* staging`, plus the refspec forms `*:main`/`*:master`/`*:develop`/`*:staging` and `git push origin +*`.
  **These globs are prefix/suffix-anchored string matches, not a refspec parser** — they cover the common literal shapes but cannot express "protected branch named anywhere in the refspec" or "force flag anywhere in the args" (`git push origin HEAD:main`, `git push origin +HEAD:main`, `git push origin main --force`, `git push origin refs/heads/main` all slip past the static list). The categorical mechanism is the `PreToolUse` Bash hook `hooks/block-protected-push.sh` (#194): it strips commit-message payloads, isolates every `git push …` segment from chained commands, resolves the actual destination ref (stripping a leading `+`/`refs/heads/`, following `src:dst` to `dst`) against `main|master|develop|staging`, and separately detects any hard-force flag or `+refspec` — blocking regardless of which surface form was used. The static globs above are belt-and-suspenders for the literal cases; the hook is what makes the deny categorical.
