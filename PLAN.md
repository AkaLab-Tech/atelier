...
- `Bash(git push * main)`, `* master`, `* develop`, `* staging`, plus the refspec forms `*:main`/`*:master`/`*:develop`/`*:staging`, `git push origin +*`, and the trailing-flag forms `* main --force`/`* main -f` (and the `master`/`develop`/`staging` equivalents, #94 partial).
  **These globs are prefix/suffix-anchored string matches, not a refspec parser** — they cover the common literal shapes but cannot express "protected branch named anywhere in the refspec" or "force flag anywhere in the args" in general (`git push origin HEAD:main`, `git push origin +HEAD:main`, `git push origin refs/heads/main` still slip past the static list, as would any less-common flag ordering or spacing). The categorical mechanism is the `PreToolUse` Bash hook `hooks/block-protected-push.sh` (#194): it strips commit-message payloads, isolates every `git push …` segment from chained commands, resolves the actual destination ref (stripping a leading `+`/`refs/heads/`, following `src:dst` to `dst`) against `main|master|develop|staging`, and separately detects any hard-force flag or `+refspec` — blocking regardless of which surface form was used, in any position. The static globs above are belt-and-suspenders for the literal cases; the hook is what makes the deny categorical.
