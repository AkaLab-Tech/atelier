...
| v2.3 | **Layer 3 of the permission model** — LLM-backed semantic judgment for commands the static matrix (§3) and M2.4 pattern hooks do not enumerate (shell composition, context-dependent destinations, novel binaries). Three implementation options under evaluation in M2.6: (A) Claude Code's native `auto` permission mode, (B) custom `PreToolUse` LLM hook with no cache, (C) both as defense in depth. The chosen option **complements** layers 1+2, does not replace them — Anthropic's auto-mode reports ~17% false negatives, so the static layer carries primary responsibility. | [`scripts/permission-handler.mjs`](https://github.com/Yeachan-Heo/oh-my-claudecode/blob/main/scripts/permission-handler.mjs) (option B reference) · [Claude Code auto-mode docs](https://code.claude.com/docs/en/permission-modes) (option A) | Catches semantic-intent attacks that pattern-based layers miss, without sacrificing the auditable allow/deny matrix |
