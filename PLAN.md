...
- All edits happen inside the task's **worktree**. The slash command `/next-task` instantiates a fresh `.claude/settings.json` from `settings.template.json`, injecting the worktree path into `additionalDirectories` and `Edit`/`Write` patterns.
  **✂️ retracted (#196a):** this used to also claim "on task close the settings revert to the base (no editable paths)". That was never actually true: `settings.json`'s `Edit`/`Write` grants are scoped to `<worktree>-worktrees/**`, a directory every sibling task's worktree also lives under, so the static permission layer cannot express "this task's worktree, not any other" — narrowing it to a single task worktree would deny every task. Several load-bearing flows also need the **main checkout** writable regardless of any task's lifecycle: `unblocker` Step 5, `task-orchestrator` Step 8's oversize terminal, `/atelier:plan-task` under `committed`/`local` planStorage, auto-merge step 4, and `/adopt-roadmap` / `/atelier:release` / `/atelier:setup-project` / `/atelier:align`. The isolation this line promised is enforced instead by the categorical `PreToolUse` hook `hooks/block-cross-worktree-edit.sh` (see §3) — a session that already owns one worktree is denied editing a sibling it does not own, while a session that owns nothing (the operator's own troubleshooting reads/edits) is never denied. This design change was flagged to and approved by the operator directly.
...
**Defense-in-depth.** This matrix is the **first** security layer: it gates *which tool* an agent can invoke. It does **not** inspect the *content* the tool would act on — `Edit(<worktree>/**)` is allowed for any file in the worktree regardless of what is being written to it, and (per §1's retraction, #196a) it cannot express "this task's worktree, not any sibling task's worktree" at all, since every worktree lives under the same `<worktree>-worktrees/**` parent. The **second** layer is the `PreToolUse` hook suite delivered in M2.4 (`scan-edit-write`, `scan-git-add`, `safe-package-change`, `block-env-commit`, `safe-commit`) plus `block-protected-push` (#194) and `block-cross-worktree-edit` (#196a): those hooks intercept allowed tool calls and validate intent — proposed file contents, staged git diff, `package.json` changes before `pnpm install`/`add`/`update`/`run`, the actual resolved git-push destination, and (for `block-cross-worktree-edit`) which worktree a session already owns before it touches a sibling's. A **third** layer — LLM-backed semantic judgment for commands layers 1+2 cannot enumerate (shell composition, context-dependent destinations, novel binaries) — is deferred; the A/B/C choice (native auto-mode, custom `PreToolUse` hook, or both) is the M2.6 spike, and the chosen implementation lands as §11 v2.3. Neither layer alone is enough — a leaky static matrix would let through forbidden tools, and a leaky hook layer would miss tools that should never have been invoked. Both must hold for a real attack to land.
...
- `Bash(git push --all*)`, `git push * --all*`, `git push --mirror*`, `git push * --mirror*` — bulk pushes update (and, for `--mirror`, delete) *every* remote ref, protected branches included, without ever naming one (#35). `--tags` stays allowed: tag pushes are sanctioned by the release flow and cannot move a branch ref.
- `Bash(git push * main)`, `* master`, `* develop`, `* staging`, plus the refspec forms `*:main`/`*:master`/`*:develop`/`*:staging`, `git push origin +*`, and the trailing-flag forms `* main --force`/`* main -f` (and the `master`/`develop`/`staging` equivalents, #94 partial).
  **These globs are prefix/suffix-anchored string matches, not a refspec parser** — they cover the common literal shapes but cannot express "protected branch named anywhere in the refspec" or "force flag anywhere in the args" in general (any less-common flag ordering or spacing still slips past the static list). The categorical mechanism is the `PreToolUse` Bash hook `hooks/block-protected-push.sh` (#194): it strips commit-message payloads, isolates every `git push …` segment from chained commands, unquotes each token (so `origin "task/x:main"` cannot hide behind shell quoting, #35), resolves the actual destination ref (stripping a leading `+`/`refs/heads/`, following `src:dst` to `dst`) against `main|master|develop|staging`, and separately detects any hard-force flag, `+refspec`, or bulk `--all`/`--mirror` — blocking regardless of which surface form was used, in any position. The static globs above are belt-and-suspenders for the literal cases; the hook is what makes the deny categorical.
  **Known residual gap:** an argument-less `git push` takes its destination from the branch's configured upstream, which a pure string parser over `tool_input.command` cannot resolve. The hook allows it rather than shelling out to `git` (which would break its hermeticity); the worktree's `task/*` branch never has a protected upstream, so this is accepted.
...
- Any `Edit`/`Write` outside the current task's worktree.
  **These grants are `<worktree>-worktrees/**`-scoped, not per-task.** The static permission layer cannot express "this task's worktree only" — every sibling task's worktree lives under the same parent directory, so a glob narrow enough to exclude siblings would exclude the task's own worktree too (§1's retraction, #196a). The categorical mechanism is the `PreToolUse` hook `hooks/block-cross-worktree-edit.sh` (#196a): it self-claims a worktree for the calling session on first write (keyed per worktree, not per session — one `/atelier:babysit-prs` pass may own many at once) and denies a session from editing a **different**, freshly foreign-owned worktree of the same repository while it already owns one of its own. A session that owns nothing yet — the operator's own troubleshooting reads/edits from outside any claimed worktree — is never denied. Freshness is a two-sided liveness proxy, not a bare mtime-since-claim: the same script also runs as a `Stop` hook that releases every claim the ending session held (resolving the dead-owner lockout the moment a task chain finishes normally, rather than waiting out `ownerTtlSeconds`), and an owner's own accepted writes refresh its claim's mtime so an actively-used claim never ages into "stale" purely from wall-clock time. A crash mid-turn never fires `Stop`, so `ownerTtlSeconds` remains the fallback net for that case — no reliable pid-based liveness check was found to survive a `Task` subagent dispatch in this harness. The session-token source (`bridge-env` > payload `session_id` > transcript-parent-dir > none) is logged on every decision, since nothing in this repo sets `CLAUDE_CODE_BRIDGE_SESSION_ID` today and a real run may always be resolving the weaker, per-subagent-unstable `session_id` fallback with no other visible sign of it; `atelier-doctor` surfaces the live-session view. Same residual gap class as `block-protected-push`: bash-mediated writes (in-place stream edits, shell redirection, `mv`/`cp`) bypass the hook, since it only intercepts `Edit`/`Write`/`MultiEdit`/`NotebookEdit`.
...
- `/abandon-task <id> [--yes|-y]` — stop and discard an in-flight task: close its open PR without merging, remove the worktree and branch, move tracking to a terminal state. The inverse of `/next-task`'s claim; preserves the approved plan and task for re-claiming — not a permanent kill.
...
### 14.9 `/atelier:release` command ✅

Captured 2026-07-03 (#43). Ships a first-class `/atelier:release [<version>|major|minor|patch] [--project <path>] [--all] [--yes|-y]` slash command that cuts the §14.1 per-PR release through the **existing independent chain** — it does not introduce a new merge path or a GH Actions workflow (that remains the separate, still-deferred M5.0.5 idea in §14.8).

- **Resolution is a pure helper.** `scripts/atelier-release resolve --project <path>` reads `.claude-plugin/plugin.json:version` (current) plus Conventional-Commit prefixes on `origin/main` since the last `vX.Y.Z` tag, and infers the §14.2 bump (`feat` → minor; `fix`/`chore` → patch; `!` after the type or `BREAKING CHANGE` in a body → major). An explicit `<version>`/`major`/`minor`/`patch` argument overrides inference. It refuses (non-zero exit) on a dirty or behind-`origin/main` project, or when the resolved version does not advance past current; it no-ops (exit 0, no chain) when there are zero unreleased commits.
- **The command never authors the bump PR from its own turn.** It dispatches the same independent `pr-author` (AtelierAuthor) → `reviewer` (AtelierReviewer) → auto-merge chain every other atelier PR goes through, unchanged — reusing the existing agents/skill rather than adding a release-specific author or reviewer. This preserves the guarantee behind the two-identity split (§ Phase 5 M5.0.1): the harness classifier blocks an agent from approving its own PR, and the author identity lacks repo-admin, so authoring and reviewing a release bump must stay two separate dispatches exactly as they do for a normal task.
- **Branch shape.** The bump commit rides `task/release-<version>` — not `release/*` or `hotfix/*` — because `pr-flow` and the permission matrix (§3) only allow pushes to `task/*`. This is the release command's own convention, distinct from the ordinary `task/<id>-<slug>` shape (a release has no roadmap/board item).
- **No tracking move.** `pr-author` runs in `follow_up: true` / no-tracking mode for a release PR: there is no board item, so there is no `IN_PROGRESS → HISTORY` (or backend `moveTask`/`appendHistoryEntry`) move to make. `pr-author` itself is unchanged — the command's briefing simply omits a task record.
- **Tag push happens only after merge**, never before: `git fetch origin main` → `git tag -a v<version> <merge-sha> -m "atelier v<version>"` → `git push origin v<version>`, authorized by two new tightly-scoped `templates/settings.template.json` allow entries (`Bash(git tag -a v*)`, `Bash(git push origin v*)`) alongside the existing `Bash(git push origin task/*)` rules. Neither pattern can match a branch push — atelier's task branches are always `task/*`, never `v*` — and neither is shadowed by the `Bash(git push * main|master|develop|staging)` deny rules, which require a literal branch-name suffix. After the tag lands, the command prints the operator update recipe (`claude plugin marketplace update akalab-tech` → `claude plugin update atelier@akalab-tech` → `atelier-update`).
- **Scope**: single project per invocation. `--all` (fan out across every registered project/workspace member with unreleased commits) is a documented, reserved follow-up — tracked as #44 — not implemented by this command; `scripts/atelier-release enumerate` exists only as a stub that reports "not yet implemented (#44)".

This does not change §14.1's trigger (still per-PR merge to `main`) or §14.2's SemVer mapping — it only replaces the *manual* `gh release create` / manual tag steps with a driven, auditable command while keeping the actual merge on the same reviewed, guarded path as any other PR.
