...
- [ ] `bug` `pr-author` force-deletes the remote task branch (`git push origin --delete`) to clean a dirty branch — the auto-mode classifier blocks it as `[Git Destructive]`, stalling the chain `#33` `~TODO` — [detail](roadmap/TASK_033_pr-author-destructive-remote-delete.md)
