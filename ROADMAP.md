...
- [ ] `bug` `/atelier:align` Tier 3 ignores `decisionPolicy=auto` — stops to ask (off-spec reviewer prompt, merge prompt + spurious `gh pr merge` permission deflection, manual post-merge pull) instead of running autonomously `#30` `~TODO` — [detail](roadmap/TASK_030_align-tier3-ignores-auto-policy.md)
