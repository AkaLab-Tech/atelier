...
- [ ] `bug` `operator-language.test.sh` flaky under pipefail: `printf | grep -q` SIGPIPE over the ~26KB hook output reds-out unrelated PRs in the `structural` CI job (fix: here-string match) `#36` `~TODO` — [detail](roadmap/TASK_036_operator-language-test-flaky-sigpipe.md)
