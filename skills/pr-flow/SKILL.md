...
Bookkeeping lines stay out of the count by construction, which is the intended reading of the budget: the budget measures reviewable change, and the `IN_PROGRESS → HISTORY` move is not it. Measuring inside `auto-merge` would be too late (the reviewer already spent their cycle).
