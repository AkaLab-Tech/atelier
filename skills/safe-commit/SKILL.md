...
The `✓ / ✗` symbols and the `Result: GREEN | RED` line are the parser's anchor. Keep them stable. The `Next step:` line on a green report is mandatory: it exists to stop callers — `pr-author` especially — from reading `GREEN` as "done" and ending their turn at the gate.
