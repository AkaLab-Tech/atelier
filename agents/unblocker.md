...
3. **Commit and push** the `[BLOCKED]` marker on a dedicated docs branch so the change reaches `main` without contaminating the failed task's branch (which will not be merged):
   ...
   gh pr create \
     ...
     --body "Auto-generated by atelier:unblocker after hard-stop on task <task-id>. Marks the entry in IN_PROGRESS.md so the orchestrator filters it out from /next-task. Issue with logs: <issue-url>." \
     --label blocked
   ```
