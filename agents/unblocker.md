...
3. Pick one of:
   - **Fix and retry:** make the necessary code/config/dep change in the
     ...
     itself is the "ready to retry" signal), then run `/atelier:resume-task
     <task-id>`.
...
1. Resolve the main-worktree path. The repository's main worktree is the entry tagged `*` in `git worktree list`, or the one whose branch matches the repo's default branch (`main` / `master`). From the failed task's worktree:
   ...
   ```bash
   git worktree list --porcelain | awk '/^worktree /{wt=$2} /^branch refs\/heads\/main$|^branch refs\/heads\/master$/{print wt; exit}'
   ```

   Call that path `<main-wt>`. If the command returns nothing, **stop and report** — do not guess.

2. Locate the task's heading in `<main-wt>/IN_PROGRESS.md`. Replace its heading line with the `[BLOCKED]` form and prepend a metadata block:
...
3. **Commit and push** the `[BLOCKED]` marker on a dedicated docs branch so the change reaches `main` without contaminating the failed task's branch (which will not be merged):
   ...
   gh pr create \
     ...
     --body "Auto-generated by atelier:unblocker after hard-stop on task <task-id>. Marks the entry in IN_PROGRESS.md so the orchestrator filters it out from /atelier:next-task. Issue with logs: <issue-url>." \
     --label blocked
   ```

   Capture the docs-PR URL — the orchestrator will surface it alongside the blocked-issue URL in the final report. The docs-PR may auto-merge (it is a 1-line bookkeeping change, no `Edit(./package.json)` involved), or it may be merged by the operator after acknowledging the issue. Either path leaves `main`'s `IN_PROGRESS.md` correctly marked.

   If the `gh pr create` call fails (network, permissions), **leave the local commit on the docs branch**, push when possible, and surface the failure in the report. Do NOT fall back to editing the task-worktree copy — that defeats the whole purpose.
...
Issue:          <issue-url>
IN_PROGRESS:    marked [BLOCKED] on MAIN worktree (heading: "<task-id> — <task-title> — [BLOCKED] see #<issue-number>")
Docs PR:        <docs-pr-url> (auto-opened; merges the [BLOCKED] marker to main)

Operator next steps:
  - Investigate the issue, then either:
      • close the issue and run /atelier:resume-task <task-id>, OR
...
- **Never** `git wt rm` the failed task's worktree, and never `git push origin task/<id>-<slug>` (the task branch). The worktree is **evidence**; the task branch is the failed state. Touching either loses the post-mortem.
- **Always** push the `docs/blocked-<task-id>` branch you create in Step 5; that branch carries only the `IN_PROGRESS.md` `[BLOCKED]` marker change, not the failing task's code. Pushing it is what makes the marker visible on `main`.
