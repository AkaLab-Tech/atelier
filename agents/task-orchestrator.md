...
- PR: `<url>`.
- Status: `merged (<sha>)` | `held — <guardrails that failed>` | `held: reviewer approval blocked by auto-mode classifier — human merge required` | `request-changes (N findings)` | `review-fix exhausted — <N> cycles, <total findings> findings accumulated; see .task-log/*.md` | `oversized — <lines>/<files>, branch task/<id>-<slug> pushed without PR` | `blocked — see <issue-url>` on hard stop.
