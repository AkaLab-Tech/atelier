...
3. **Move tracking forward inside the task worktree.** (Skip in resume mode — the entry is already in `IN_PROGRESS.md`.) **Critical scope rule (M4.8 / Findings #13 + #17):** edit the `ROADMAP.md` and `IN_PROGRESS.md` that live **inside the per-task worktree** (the one you just created in step 2), NOT the copies in the main worktree. Move the chosen task's block from `ROADMAP.md` to `IN_PROGRESS.md` in a single edit, per the `roadmap-tracking-flow` convention (or the project's local layout if different). The new entry coexists with any pre-existing `[BLOCKED]` entries — do not overwrite or reorder them. The reason: the per-task worktree is on the `task/<id>-<slug>` branch, so this edit becomes part of the task branch; the squash-merge later brings both this move and `pr-author`'s `IN_PROGRESS → HISTORY` move into `main` in a single commit, honouring `roadmap-tracking-flow`'s "same PR" rule. Editing the main worktree's copy here would leave `main` with uncommitted bookkeeping that no agent is allowed to push.
