...
- [x] **Phase C.2** — drive Claude Code via the `claude plugin` CLI verbs (the slash-command form `/plugin …` only works inside an interactive session; `claude plugin …` achieves the same flow from a shell script): `claude plugin marketplace add AkaLab-Tech/claude-plugins` + `claude plugin install atelier@akalab-tech` + `claude plugin install claude-roadmap-tools@akalab-tech`. Idempotent via `claude plugin marketplace list --json` and `claude plugin list --json` (parsed with `jq`, already a Phase A dep). Guard: skip Phase C.2 with a manual-fallback message when `claude` is missing from PATH or `claude auth status` reports unauthenticated (typically because Phase B skipped due to no TTY); rest of install.sh still runs.
- [x] **Final verification block** — `phase_verify()` reports `✓`/`✗` per check (Unicode glyphs, not emojis) for: `claude --version`, `claude auth status`, `gh auth status`, `git wt help`, `atelier@akalab-tech` plugin installed, `claude-roadmap-tools@akalab-tech` plugin installed. `/doctor` invocation is deferred with an inline note — that slash command lands with M1.6 / M2.x. Each check runs quietly and never aborts the script; verification is reporting-only. Final log message also updated to point operators at `source ~/.zshrc` for `task` / `task-status` to become live.
- [x] **Idempotency** — verified via the no-TTY and TTY smoke tests of every sub-PR (#8, #12, #13, #14, this one): each phase short-circuits on already-configured state with a clear "already installed / already added / already present" sublog, and the new verification block at the end gives the operator an unambiguous green/red summary. No separate stamp file — per-step idempotency + verification cover the ROADMAP requirement.

**Acceptance:** running `install.sh` on a clean Mac VM (and best-effort on Ubuntu) finishes with all final checks green.

**Branch (current sub-PR):** `setup/m1.3-final` — final verification block + idempotency wrap-up; closes M1.3 by moving this block to `HISTORY.md` in an atomic follow-up commit on this same branch.
