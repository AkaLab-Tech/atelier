...
**Acceptance criterion status:** **structurally satisfied + partially empirically validated**. Pre-merge `--plugin-dir` validation confirms the plugin-level `.mcp.json` is loaded by the harness and exposes all 23 tools under `mcp__plugin_atelier_playwright__*`. Per-agent restriction (subagent `implementer` sees the tools, subagent `tester` does not) and deny enforcement (`browser_run_code_unsafe` blocked) require a project with the instantiated `settings.template.json` and remain deferred to the next UI-touching task post-merge.
