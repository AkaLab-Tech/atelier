...
### TASK_022b — M9.5b: Mixed-backend workspace status aggregation in atelier-workspace-status — 2026-06-23
**PR:** [#235](https://github.com/AkaLab-Tech/atelier/pull/235) (native atelier-dev; built on #22a M9.5a backend-aware resolver, PR #233)

Second deliverable of the M9.5 epic (#22). `atelier-workspace-status` is now mixed-backend-aware: per-member backend detection via `atelier-task-backend` (M_BACKENDS array); non-files members emit `backend:<name>` in the open-tasks column instead of the misleading `0` that appeared before; JSON output gains a `backend` field per member with `openTasks: null` for non-files members (distinguishes "zero open" from "unknown"). `member_xrepo_blocked` interprets `atelier-resolve-dep` exit 6 (backend-deferred, introduced in #22a) as shown+unsatisfied, so the blocker is counted correctly rather than silently dropped. Non-files members are excluded from the §5 drift false-alarm check. The files-only render path is byte-identical; only the additive `backend` JSON field differs. New `hooks/tests/workspace-status-mixed-backend.test.sh` (25 assertions across 5 groups: mixed-backend degradation, backend JSON field, exit-6 handling, drift exclusion, files-only no-regression) auto-discovered by structural.yml. Realises the "aggregates status" half of the M9.5 parent acceptance (PLAN.md §16.7). Closes #22b; advances epic #22.
...
### TASK_020b — M9.3b: next-task / task-discovery read + claim against the GitHub Project backend — 2026-06-22
**PR:** [#226](https://github.com/AkaLab-Tech/atelier/pull/226) (native atelier-dev; built on #20a M9.3a + #18 M9.1 + #19 M9.2)

Second slice of the M9.3 epic (#20). Tightens `commands/next-task.md` (steps 2/3/6) and `skills/task-discovery/SKILL.md` so the `github-project` backend is concretely specified end-to-end. Step 2: backlog obtained via `listTasks("roadmap")` + `githubProject.stateMap` bucket mapping; Project In-Progress column explicitly NOT the concurrency signal — claim registry remains open `task/*` PRs, backend-agnostic. Step 2: `Ready` field named as the planning-gate signal, distinguished from the files-backend `[ready]` text token. Step 3: `Ready` field rides along in the `listTasks` record — no extra backend call (resolves the plan's open question, grounded against crt v0.5.0 `docs/RoadmapBackend.md`); planning gate = Ready set AND `.plan/<id>.md` committed. Step 6: claim drives `moveTask(id,"roadmap","in_progress")` + `githubProject.stateMap.inProgress` Status transition; no local ROADMAP/IN_PROGRESS edits for non-files backends. `task-discovery` SKILL.md: Priority single-select field mapping (P0/P1/P2 identity) + Ready-field gate documented in the Backend-aware section. Files-backend no-regression: `[ready]` marker + open `task/*` PR claim registry preserved unchanged. New `hooks/tests/next-task-github-project-claim.test.sh` (22 assertions across 5 groups) auto-discovered by `structural.yml`'s glob loop — no workflow edit. Full hermetic suite 16/16 green. Siblings: #20a (merged, #224) / #20c (open).
