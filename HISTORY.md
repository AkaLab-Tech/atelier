...
- `templates/settings.template.json` — `permissions.allow` gains `mcp__playwright__*` (one entry), so the harness does not prompt the operator on each tool call once the agent has the tool in its `tools:` list. `permissions.deny` gains a single entry `mcp__playwright__browser_run_code_unsafe` to block the one MCP tool that is RCE-equivalent on the operator's host (see Decisions captured below).
...
- Frontmatter check: `agents/implementer.md:25` and `agents/reviewer.md:34` list `mcp__playwright` in `tools:`; the other 5 agents (`tester`, `e2e-runner`, `pr-author`, `task-orchestrator`, `unblocker`) do not.
