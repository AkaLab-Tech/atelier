...
- `author_agent: pr-author` — **required, not optional.** The release branch `task/release-<next_version>` is `task/*`-shaped, and #44a's non-task-pr coordination mode otherwise defaults the authoring primitive to `pr-opener`, which is scoped to `chore/*` / `docs/*` / `fix/*` / plan-tracking branches — never `task/*`. `pr-author` is the primitive that already handles `task/*` branches, so this hint overrides that default and routes authoring to the correct agent for this branch shape.
- `interactive: <bool>` from this command's interaction mode.
...
- **Never** omit the `author_agent: pr-author` hint from the step 7 briefing — without it, non-task-pr mode defaults the authoring primitive to `pr-opener`, which is scoped to `chore/*`/`docs/*`/`fix/*`/plan-tracking branches and does not handle the `task/release-<version>` branch shape this command always uses.
