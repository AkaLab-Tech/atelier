...
argument-hint: "[--yes|-y] [--mode=new|existing] [--no-branch-protection] [--apply-branch-protection] [--backend <files|linear|github-project>] [--scaffold-ci] [project-path-if-not-cwd]"
...
10. Applies branch protection **by default** (#45) by delegating to the shared `atelier-branch-protection` helper: classifies the default branch's rule (`protected-sufficient` / `protected-insufficient` / `unprotected` / `protected-noadmin` / `no-admin`) and, on any gap, resolves an admin-capable `gh` identity (atelier admin → atelier author/bot → the operator's personal `gh` config) and applies an idempotent, merge-not-overwrite rule (`required_approving_review_count>=1`, `enforce_admins=false`, existing stronger settings preserved) under that identity — never prompting, and never guessing at a field it does not model (it refuses rather than risk a full-replace PUT silently dropping something the existing rule sets). There is no flag→policy→TTY-prompt ladder any more: pass `--no-branch-protection` to opt out entirely; `--apply-branch-protection` is accepted only as a deprecated no-op for backward compatibility (the step it used to gate is now unconditional). On a gap the helper cannot close automatically (no admin identity resolves, or it refuses on an unmodeled field), it prints the exact `gh api -X PUT` command the operator can run manually. Never fails setup.
