...
10. Branch protection is **detected and reported by default, NOT applied** (#45 cycle 7), by delegating to the shared `atelier-branch-protection` helper's `--status` mode: classifies the default branch's rule (`protected-sufficient` / `protected-insufficient` / `unprotected` / `protected-noadmin` / `no-admin`) and reports it — the write path (the `PUT` payload the helper's `--apply` mode would build) is **unverified** against GitHub's real API contract, so it does not run unless the operator explicitly opts in. Pass `--apply-branch-protection` to actually apply: on any gap, that opt-in resolves an admin-capable `gh` identity (atelier admin → atelier author/bot → the operator's personal `gh` config) and applies an idempotent, merge-not-overwrite rule (`required_approving_review_count>=1`, `enforce_admins=false`, existing stronger settings preserved) under that identity — never prompting, and never guessing at a field it does not model (it refuses rather than risk a full-replace PUT silently dropping something the existing rule sets). Pass `--no-branch-protection` to skip the check entirely (no detection, no report, no apply). Never fails setup on any path.
