...
4. Writes `<path>/.claude/settings.json` from `$CLAUDE_PLUGIN_ROOT/templates/settings.template.json` with two placeholders substituted: `<worktree>` → project path and `<atelier-config-dir>` → `$ATELIER_CONFIG_DIR`. Validates the result parses with `jq empty` and that no literal `<worktree>` / `<atelier-config-dir>` tokens remain.
