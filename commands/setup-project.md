...
5. Writes `<path>/.atelier.json` from `$ATELIER_CONFIG_DIR/templates/atelier.template.json` **only when missing** (M7.1.F27). This file is operator-owned after creation — it carries per-project overrides for `prSize.{maxLines,maxFiles,exempt}` (the auto-merge size budget enforced by `atelier-pr-size-check`). To reset to defaults, delete the file and re-run setup-project.
