...
- **Never** use `rm -r`, `rm -rf`, `rm -fr`, or any of its enumerated deny-list equivalents (`-Rf`, `-fR`, separated `-r -f`, long `--recursive --force`, the `-rfv` bundles, etc. — see `templates/settings.template.json`) on the worktree path. `git worktree remove --force` is the correct, git-aware removal — a raw recursive delete leaves git's internal worktree bookkeeping (`.git/worktrees/<name>`) dangling.
