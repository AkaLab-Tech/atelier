...
- Resolve the board item and its content type **from the local cache, never by listing the board**: `atelier-project-cache get <id> --no-refresh` returns the item, whose `type` column says whether it is `Issue`- or `DraftIssue`-backed. `/status` is a read-only dashboard and must not pay for a full `gh project item-list` sweep (~607 GraphQL points on this project's board) to answer a question the cache already holds. `--no-refresh` is deliberate: this command reports state, it does not own the cache's refresh cadence.
- For each open `task/*` PR, resolve the board item its branch's `<id>` corresponds to, then check whether that item has a **parent** issue (`Issue.parent` — the reverse of `subIssues`, same native-sub-issue linkage documented in `skills/task-discovery/SKILL.md` § "Backend-aware backlog source (M9.1)" step 6). Only issue-backed items can have one; a `DraftIssue`-backed item (the common case for atelier-managed boards — 594 of 596 items on this repo's own board) has no parent to check — skip it silently on the cached `type` alone, with no GraphQL call at all.
